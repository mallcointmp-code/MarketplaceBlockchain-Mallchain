...
export default function XTasks() {
  const navigate = useNavigate();
  const [allTasks, setAllTasks] = React.useState([]);

  const defaultTasks = [
    { id: 1, title: 'Tweet and earn mallpoints', reward: '0.059 mlpts' },
    { id: 2, title: 'Retweet Mallcoin posts', reward: '0.1 mlpts' },
  ];
  ...
  React.useEffect(() => {
    ...
    if (miningTasks['x'] && Array.isArray(miningTasks['x'])) {
      miningTasks['x'].forEach(task => {
        tasks.push({
          id: task.id,
      ...
      <div style={{ flex: '1', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
        <div className="p-6" style={{ flex: '0 0 auto', overflowY: 'auto' }}>
          ...
          <div className="flex items-center gap-6 mb-12">
            ...
            />
            <div>
              <h1 className="text-4xl font-bold text-white mb-2">X Tasks</h1>
              ...
              <div key={task.id}>
                ...
                >
                  ...
                  >
                    ...
                    {task.isUserCreated && (
                      <p className="text-amber-300 text-xs">• Created by You</p>
                    )}
                  </div>
                </div>
                <p className="text-purple-400 font-bold text-sm" style={{ marginTop: '8px' }}>
                  {task.reward}
                </p>
              </div>
            ))}
