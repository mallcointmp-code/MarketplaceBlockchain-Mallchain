...
func serializeSettlement(req types.SettlementRequest) []byte {
	...
	off := 0
	binary.BigEndian.PutUint64(b[off:off+8], req.Id)
	off += 8
	b[off] = byte(len(from))
	off++
	copy(b[off:off+len(from)], from)
	off += len(from)
	b[off] = byte(len(to))
	off++
	copy(b[off:off+len(to)], to)
	off += len(to)
	binary.BigEndian.PutUint64(b[off:off+8], req.Amount)
	off += 8
	binary.BigEndian.PutUint64(b[off:off+8], req.EnqueueHeight)
	off += 8
	binary.BigEndian.PutUint32(b[off:off+4], req.RequiredSigs)
	off += 4
	binary.BigEndian.PutUint32(b[off:off+4], req.Confirmations)
	off += 4
	if req.Completed {
		b[off] = 1
	} else {
...
func deserializeSettlements(raw []byte) ([]types.SettlementRequest, error) {
	...
	for i < len(raw) {
		...
		id := binary.BigEndian.Uint64(raw[i : i+8])
		i += 8
		if i+1 > len(raw) {
			return nil, errors.New("corrupt")
		}
		...
		to := string(raw[i : i+tl])
		i += tl
		if i+8+8+4+4+1 > len(raw) {
			return nil, errors.New("corrupt")
		}
		amount := binary.BigEndian.Uint64(raw[i : i+8])
		i += 8
		enqueue := binary.BigEndian.Uint64(raw[i : i+8])
		i += 8
		reqSigs := binary.BigEndian.Uint32(raw[i : i+4])
		i += 4
		conf := binary.BigEndian.Uint32(raw[i : i+4])
		i += 4
		completed := raw[i] != 0
		i++
...
func serializeSettlements(list []types.SettlementRequest) []byte {
	var out []byte
	for _, r := range list {
		out = append(out, serializeSettlement(r)...)
	}
	return out
}
...
func (k Keeper) EnqueueTransfer(ctx context.Context, from, to string, amount uint64) (bool, error) {
	...
	var seq uint64
	if len(seqB) >= 8 {
		seq = binary.BigEndian.Uint64(seqB[:8])
	}
	seq++
	var nb [8]byte
	binary.BigEndian.PutUint64(nb[:], seq)
	_ = s.Set(keySettlementSeq, nb[:])

	// compute unlock height
	var enqueueH uint64
	if sdkCtx, ok := ctx.(sdk.Context); ok {
		enqueueH = uint64(sdkCtx.BlockHeight())
	}
	...
	if err := s.Set(keySettlementQueue, raw); err != nil {
		return false, err
	}
	return false, nil
}
...
func (k Keeper) ProcessSettlementQueue(ctx sdk.Context) error {
	...
	for _, r := range list {
		...
		if currentH < r.UnlockHeight {
			remaining = append(remaining, r)
			continue
		}
		if r.Confirmations < r.RequiredSigs {
			remaining = append(remaining, r)
			continue
		}
